...
class Jantar():
    def __init__(self, janela, player):
        ...
        self.memoria = Memoria(janela)
        self.interativo_memoria = False
    ...
    #===============================================================================================#
        if Window.keyboard.key_pressed("e") and self.player.collided(armario):
            self.interativo_memoria = True

        if self.interativo_memoria:
            self.janela.set_background_color((100,0,0))
            self.memoria.mostrar_obejtos()
        #PRECISA VERIFICAR QUANDO GANHA!!!!!!!!!!!!!!!!!!!!
